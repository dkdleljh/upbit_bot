...
### 원칙: **키/토큰은 절대 Git에 커밋하지 않습니다**
- `.env` / `~/.upbit_bot.env` / 쉘 환경변수에만 저장하세요.
- 이 레포에는 **키가 없는 템플릿**(`.env.example`)만 포함됩니다.
...
# 2) .env에 실제 키 입력 (로컬에서만)
#    ⚠️ .env는 반드시 .gitignore에 의해 추적되면 안 됩니다.
nano .env

# 3) 확인: .env가 staged/tracked가 아니어야 함
git status
```

**추가 보안 권장사항:**
- 실거래 운영 키는 `~/.upbit_bot.env` 같은 **홈 디렉토리의 별도 파일**에 저장하고 쉘에서 로드
- 키 권한 최소화(출금 권한 OFF)
- 키 정기적 교체
- 필요하면 IP 화이트리스트 설정

> ⚠️ 한 번이라도 키가 Git 히스토리에 들어가면(나중에 삭제 커밋을 해도) 남습니다. 그 경우 키 폐기/재발급이 필요합니다.
