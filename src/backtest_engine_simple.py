...
class HistoricalDataLoader:
    def __init__(self, storage: Storage):
        ...
        self.candles_cache: Dict[str, List[Dict[str, Any]]] = {}
        ...
        if cache_key not in self.candles_cache:
            try:
                ...
                rows = self.storage.query(
                    query, (market, start.timestamp(), end.timestamp())
                )
...
class BacktestEngine:
    def __init__(
        self, cfg: dict[str, Any], storage: Storage, data_loader: HistoricalDataLoader
    ):
        self.cfg = cfg
        self.storage = storage
    ...
    async def run_backtest(self) -> BacktestResult:
        LOGGER.info(
            f"Starting backtest from {self.backtest_config.start_date} to {self.backtest_config.end_date}"
        )
    ...
    async def _process_exits(self, current_time: datetime):
        for market, position in list(self.portfolio.positions.items()):
            ...
            if exit_reason:
                exit_result = self._simulate_exit_order(
                    ...
                    self.all_trades.append(
                        {
                            ...
                            "pnl": self._calculate_trade_pnl(
                                position, exit_result.fill_price
                            ),
                            "exit_reason": exit_reason,
                            "hold_seconds": (
                                current_time
                                - datetime.fromtimestamp(position.entry_ts_ms / 1000)
                            ).total_seconds(),
                            "slippage": exit_result.slippage_pct,
                        }
                    )

    async def _process_entries(self, signals: List[Any], current_time: datetime):
        ...
        for signal in viable_signals:
            ...
            position_value = self.risk_manager.compute_position_value(
                ...
                coin_exposure_now=self.portfolio.coin_exposure_ratio(
                    parse_market(signal.market).base, self.risk_manager.equity, {}
                ),
                signal_score=max(85.0, 85.0 + (signal.score - signal.dynamic_cutoff)),
            ...
            entry_result = self._simulate_entry_order(
                signal, entry_price, position_value
            )
    ...
    def _simulate_entry_order(
        self, signal, entry_price: float, position_value: float
    ) -> ExecutionResult:
        mock_orderbook = {
            "orderbook_units": [
                {
                    "ask_price": entry_price * 1.001,
                    "ask_size": position_value / entry_price,
                },
                {
                    "bid_price": entry_price * 0.999,
                    "bid_size": position_value / entry_price,
                },
            ]
        }
        ...
        slippage = self.slippage_simulator.calculate_slippage(
            ...
            mock_orderbook,
            signal.notional_ratio,
        )

        fill_price = entry_price * (1 + slippage)
        market_info = parse_market(signal.market)
        commission_rate = self.backtest_config.commission_rates.get(
            market_info.quote, 0.0005
        )
    ...
    def _simulate_exit_order(
        self, position: Position, current_price: float, exit_reason: str
    ) -> ExecutionResult:
        mock_orderbook = {
            "orderbook_units": [
    ...
    def _calculate_recovery_factor(self) -> float:
        ...
        max_dd_value = (
            self._calculate_max_drawdown() * self.backtest_config.initial_equity
        )
    ...
    async def _generate_signals(
        self, universe: List[str], current_time: datetime
    ) -> List[Any]:
        signals = []
        ...
        for market in universe:
            candles = self.data_loader.load_candles(
                market, current_time - timedelta(hours=2), current_time
            )

            if len(candles) < 24:
                continue

            notional_ratio, spread_pct, depth_ratio = self._build_quality_proxies(
                candles
            )
    ...
    def _build_quality_proxies(
        ...
        volume = float(
            candles[-1].get("volume", candles[-1].get("candle_acc_trade_volume", 0.0))
            or 0.0
        )

        close_values: List[float] = [
            float(c.get("close", c.get("trade_price", 0.0))) for c in candles
        ]
        ...
        for c in candles:
            ...
            if value <= 0:
                v = float(c.get("volume", c.get("candle_acc_trade_volume", 0.0)) or 0.0)
                p = float(c.get("close", c.get("trade_price", 0.0)) or 0.0)
                value = p * v
            notional_values.append(value)

        lookback = min(20, len(notional_values))
        recent_notional = notional_values[-lookback:]
        recent_volume = volume_values[-lookback:]

        avg_notional = sum(recent_notional) / lookback if lookback else 1.0
        avg_volume = sum(recent_volume) / lookback if lookback else 1.0

        notional_ratio = notional_values[-1] / avg_notional if avg_notional > 0 else 0.0
        depth_ratio = volume / avg_volume if avg_volume > 0 else 0.0

        notional_ratio = max(0.0, min(notional_ratio, 20.0))
        depth_ratio = max(0.1, min(depth_ratio, 20.0))

        if close <= 0:
            spread_pct = 0.001
        else:
            candle_range = max(0.0, high - low)
            spread_pct = candle_range / close
            spread_pct = min(max(spread_pct * 0.12, 0.00005), 0.01)

        atr_pct = atr_percent(candles, 14)
        if atr_pct > 3.0:
            spread_pct = min(spread_pct * 1.4, 0.01)
        elif atr_pct < 0.5:
            spread_pct = spread_pct * 0.8

        return notional_ratio, spread_pct, depth_ratio
