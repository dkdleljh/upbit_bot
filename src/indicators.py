...
def stochastic_rsi(closes: list[float], rsi_period: int = 14, k_period: int = 3, d_period: int = 3) -> tuple[float, float]:
...
def support_resistance(candles: list[dict], lookback: int = 20) -> tuple[list[float], list[float]]:
...
def market_structure(candles: list[dict], lookback: int = 10) -> str:
    ...
    if range_pct < 2:
        if current > ema(candles[-lookback:], min(lookback, 20)) or ema(candles[-lookback:], 5):
            return 'uptrend'
        else:
            return 'downtrend'
...
def volume_profile(candles: list[dict], bins: int = 20) -> tuple[float, float, list[float]]:
    ...
    price_range = max(float(c["high"]) for c in candles) - min(float(c["low"]) for c in candles)
    ...
    poc_price = min(float(c["low"]) for c in candles) + (bin_idx + 0.5) * bin_size
...
def market_regime(candles: list[dict], lookback: int = 20) -> dict:
    if len(candles) < lookback:
        return {'trend': 'neutral', 'volatility': 'normal', 'strength': 50, 'signal': 'neutral'}
...
def momentum_score(candles: list[dict], lookback: int = 10) -> float:
    ...
    closes = [float(c["close"]) for c in candles[-lookback:]]
    volumes = [float(c.get("volume", c.get("candle_acc_trade_volume", 0.0)) or 0.0) for c in candles[-lookback:]]
    ...
    score = (price_change * 2) + (vol_ratio * 10) + (range_pct * 0.5) + (ema_momentum * 3)
